...
unsigned long NumSamples = 0;   // incremented every ADC sample, in Producer
...
unsigned long myId = OS_Id(); 
  PE1 ^= 0x02;
  ST7735_Message(2,0,"NumCreated =",NumCreated); 
  PE1 ^= 0x02;
  OS_Sleep(50);     // set this to sleep for 50msec
  ST7735_Message(2,1,"PIDWork     =",PIDWork);
  ST7735_Message(2,2,"DataLost    =",DataLost);
  ST7735_Message(2,3,"Jitter 0.1us=",MaxJitter);
  PE1 ^= 0x02;
...
unsigned long data,voltage;
	//while(1){
  ST7735_Message(0,1,"Run length = ",(RUNLENGTH)/FS); 
	//OS_Suspend();
	//}

	// top half used for Display
  while(NumSamples < RUNLENGTH) { 
...
//	int g = 0;
//	while(1){
//	ST7735_Message(0,1,"Run length = ", g++);
//	}
